...
import type { EntityRow, EventPayload, FetchEffect, PersistEffect } from '@almadar/core';

type TestBus = ServerEffectEventBus & { events: Array<{ event: string; payload?: EventPayload }> };

function makeBus(): TestBus {
    return {
        events: [],
        emit(e: string, p?: EventPayload) {
            this.events.push({ event: e, payload: p });
        },
    };
}

const FETCH_EFFECT: FetchEffect = [
    'fetch',
    'ListItem',
    {
        emit: {
...
describe('std-list offline preview — loading INIT emits ListItemLoaded', () => {
    it('fetch ListItem in `loading` state emits ListItemLoaded with 10 rows', async () => {
        ...
        await exec.executeAll([FETCH_EFFECT]);
    ...
    it('persist.create on ListItem adds a row the next fetch can see', async () => {
        ...
        const exec = new EffectExecutor({ handlers, bindings, context });
        const createEffect: PersistEffect = [
            'persist',
            'create',
            'ListItem',
    ...
    it('fetch does NOT mutate bindings — consumers read via bus event payload', async () => {
        ...
        await exec.executeAll([FETCH_EFFECT]);
        expect((bindings as { ListItem?: EntityRow[] }).ListItem).toBeUndefined();
        expect(bindings.entity).toBeUndefined();
        const loaded = bus.events.find((e) => e.event === 'ListItemLoaded');
        expect(loaded, 'ListItemLoaded carries the result instead').toBeDefined();
        expect((loaded!.payload as { data: EntityRow[] }).data).toHaveLength(5);
    });
});
